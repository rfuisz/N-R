...
alz_hetero_17 = c(0.3150501672, 0.4856020311, 0.4246593641, 0.2474167449,0.3535061795,0.4564337825,0.4387864207,0.4817572099,0.2689378758,0.4495394473,0.3088490843,0.0987604958,0.4439470871,0.14939228,0.1115641711,0.01945369753,0.3287890938,0.4933253237,0.3134787846,0.2221405665,0.3962420601,0.4237764108,0.03914899293,0.4762381524,0.2108796931,0.1228491397,0.4440208668,0.3643421229,0.08186077644,0.1133136887)
alz_homo_17 = c(0.07785953177, 0.3441693047, 0.1860806839, 0.04255931638, 0.1042450296, 0.2467106311, 0.2143811815, 0.3265593561, 0.05263861055, 0.2265385129, 0.07238337121, 0.005597760896, 0.2241448423, 0.01409109122, 0.007620320856,	0.0002664890073, 0.08740978348,	0.3936056601, 0.0751572748, 0.03180117584, 0.1478456773, 0.1857448516, 0.00133386688,	0.3161126685,	0.02671964922,	0.009603841537,	0.2178972713,	0.1131709276,	0.00421686747,	0.006094294133)
alz_logOR_17 = c(-0.15, -0.25, 0.21, 0.16, 0.09, 0.11, 0.28, 0.18, -0.09, 0.3, 0.08, 0.14, -0.23, 0.1, -0.11, -0.23, 0.09, -0.11, -0.07, 0.09, 0.07, -0.08, 0.18, -0.09, -0.07, -0.09, -0.2, -0.06, 0.1, 0.08)
alz_hetero_17_norm = alz_hetero_17
alz_homo_17_norm = 1-alz_homo_17-alz_hetero_17
alz_logOR_17_norm = abs(alz_logOR_17)
...
#find GRS improved
find_GRS_improved <-function(log_OR, homo_count, hetero_count, num_embryos, sample_size, embryo_cutoff = 3) {
  mean_embryo_GRS = array(0,sample_size)
  for(i in 1:sample_size) {
...
#find GRS improved
find_GRS_improved_high_risk <-function(log_OR, homo_count, hetero_count, num_embryos, sample_size, embryo_cutoff = 3) {
  mean_embryo_GRS = array(0,sample_size)
  for(i in 1:sample_size) {
    mother_genotype = simulate_genotype_high_risk(hetero_count,homo_count)
    father_genotype = simulate_genotype_high_risk(hetero_count,homo_count)
...
#returns a simulated genotype using heterozygote and homozygote rates in the target population. 
simulate_genotype_high_risk <- function(hetero_count,homo_count) {
  SNPs_in_model = length(hetero_count)
  genotype = array(0,SNPs_in_model)
  for(SNP in 1:SNPs_in_model) {
    zygote_distribution = c(0,hetero_count[SNP],homo_count[SNP])
    genotype[SNP] = sample(0:2,1,prob=zygote_distribution)
  }
  return(genotype)
}
